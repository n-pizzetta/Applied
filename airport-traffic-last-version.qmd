...
Which gives us the following parameters : `r format_arima_params(reorder_params(paris_ts19_auto_params$arma)[1:6])`
...
Parameters used : `r format_arima_params(paris_ts19_aic_params$min_AIC_params)`
...
-   **The Ljung-Box test**, $H_0 : Independant$,  $H_1 : Not Independant$
    ...
    If we get $p-value < 0.05$, we reject the null hypothesis.
    This suggests that the error has a significant autocorrelation at some of the lags used in the test (up to lag 24 here). Then our model would not capture adequately the time-dependency.
    Here, we have $p-value = `r p_value` > 0.5$.

-   **The residuals time series plot**,  helps us to check if our residuals look like white noise, indicating that our model has captured the underlying process accurately and suggesting that the error variances are consistent over time—a condition known as homoskedasticity. 
    In our case, the residuals fluctuate around zero without displaying any obvious patterns or systematic change in the spread over time, which is a good sign both for the model's fit and the assumption of homoskedasticity.
...
Parameters used : `r format_arima_params(paris_ts19_bic_params$min_BIC_params)`
...
Parameters used : `r format_arima_params(madrid_ts19_aic_params$min_AIC_params)`
...
Parameters used : `r format_arima_params(madrid_ts19_bic_params$min_BIC_params)`
...
Parameters used : `r format_arima_params(roma_ts19_aic_params$min_AIC_params)`
...
Parameters used : `r format_arima_params(roma_ts19_bic_params$min_BIC_params)`
...
Parameters used : `r format_arima_params(copenhagen_ts19_aic_params$min_AIC_params)`
...
Parameters used : `r format_arima_params(copenhagen_ts19_bic_params$min_BIC_params)`
...
Parameters used : `r format_arima_params(oslo_ts19_aic_params$min_AIC_params)`
...
Parameters used : `r format_arima_params(oslo_ts19_bic_params$min_BIC_params)`
