...
abline(h = mean(charles_ts19_diff), col = "red", lty = 2)
```
...
We now construct our model, taking into account our previous analysis.
...
Based on ACF, PACF and the suggested coefficient, we decide to start with an AR(1) non-seasonal component, and a seasonal difference.
...
results_AIC <- results_AIC[order(results_AIC$X6523.85810561624),]
results_AIC
...
results_BIC <- results_BIC[order(results_BIC$X6530.60866243161),]
results_BIC
```
